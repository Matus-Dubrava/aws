...
INSTANCEID="$(aws ec2 run-instances --image-id "$AMIID" --key-name mdcloudkey --instance-type t2.micro --security-group-ids "$SGID" --subnet-id "$SUBNETID" --query "Instances[0].InstanceId" --output text)"
...
echo "$INSTANCEID is accepting SSH connection under $PUBLICNAME"
echo "ssh -i mdcloudkey.pem ec2-user@$PUBLICNAME"
